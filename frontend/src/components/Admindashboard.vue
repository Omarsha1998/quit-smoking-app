...
<template>
  <div>
    <q-card
      class="q-mb-md shadow-lg"
      style="border-radius: 18px; overflow: hidden; border: 1.5px solid #b8d4b0"
    >
      ...
      <q-card-section style="background: #f5ead8">
        ...
        <div class="row q-col-gutter-md q-mb-lg">
          <div class="col-12 col-sm-6 col-md-3" v-for="card in summaryCards" :key="card.label">
            <q-card
              :style="card.style"
              style="border-radius: 14px; border: 1.5px solid; overflow: hidden"
            >
              <q-card-section class="q-pa-md">
                <div class="text-subtitle2 text-weight-medium" :style="{ color: card.labelColor }">
                  {{ card.label }}
                </div>
                <div class="text-h4 text-weight-bold q-mt-xs" :style="{ color: card.valueColor }">
                  {{ card.value }}
                </div>
              </q-card-section>
            </q-card>
        ...
        <ActivityStatsPanel :activities="activityStats" :loading-activities="loadingActivities" />
        ...
        >
          ...
          >
            ...
            <!-- Details -->
            <q-item-section>
              <q-item-label class="text-h6 text-weight-bold" style="color: #2e4a2e">{{
                user.name
              }}</q-item-label>
              <q-item-label caption style="color: #5d9460">
                <q-icon name="event" size="xs" class="q-mr-xs" />{{ user.daysSmokeeFree }} days
                smoke-free
              </q-item-label>
              <q-item-label caption style="color: #c97a8a">
                <q-icon name="block" size="xs" class="q-mr-xs" />{{ user.cigarettesAvoided }}
                cigarettes avoided
              </q-item-label>
              <q-item-label caption style="color: #5d9460">
              ...
              <q-item-label caption class="q-mt-sm">
                ...
                >
                  <q-icon name="smartphone" size="xs" class="q-mr-xs" />Today:
                  {{ user.opensToday || 0 }}
                </q-badge>
                <q-badge
                  class="q-mr-xs q-pa-xs"
                ...
                >
                  <q-icon name="calendar_month" size="xs" class="q-mr-xs" />Month:
                  {{ user.opensThisMonth || 0 }}
                </q-badge>
                <q-badge
                  class="q-pa-xs"
                  style="background: #ede0c4; color: #7a6040; border-radius: 6px"
                >
                  <q-icon name="timeline" size="xs" class="q-mr-xs" />Total:
                  {{ user.totalOpens || 0 }}
                </q-badge>
              </q-item-label>
...
import ActivityStatsPanel from './ActivityStatsPanel.vue'
...
export default {
  ...
  props: {
    ...
    activityStats: { type: Array, default: () => [] },
