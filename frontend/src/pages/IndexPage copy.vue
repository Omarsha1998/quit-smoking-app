    ...
    <template v-else>
      <div class="pf-tab-content q-pa-md">
        ...
        <template v-else-if="activeTab === 'home'">
          ...
          <q-card
            v-if="stats.totalDaysSinceQuit > 0"
            class="q-mb-md"
            style="border-radius: 16px; background: #f5ead8; border: 1.5px solid #ddc8a8"
          >
            <q-card-section>
              ...
              <div class="row q-col-gutter-sm text-center">
                <div class="col-4">
                  <div class="text-h6 text-weight-bold" style="color: #2e4a2e">
                    {{ stats.totalDaysSinceQuit }}
                  </div>
                  <div class="text-caption" style="color: #9aaa90">Days Since Quit</div>
                </div>
                <div class="col-4">
                  <div class="text-h6 text-weight-bold" style="color: #c97a8a">
                    {{ stats.smokedDaysCount }}
                  </div>
                  <div class="text-caption" style="color: #9aaa90">Days Smoked</div>
                </div>
                <div class="col-4">
                  <div class="text-h6 text-weight-bold" style="color: #5d9460">
                    {{ stats.days }}
                  </div>
        ...
        <template v-else-if="activeTab === 'tips'">
          ...
          <InfoList
            ...
            badge-color="pf-badge-green"
            text-color="pf-text-green"
            :items="HEALTH_BENEFITS"
          />
          <InfoList
            title="Risk Factors of Smoking"
            icon="warning"
        ...
        <template v-else-if="activeTab === 'track'">
          ...
          <SavingsBreakdown
            ...
            :stats="stats"
            :cigarettes-per-day="Number(cigarettesPerDay)"
            :price-per-pack="Number(pricePerPack)"
          />
          <HealthTimeline
        ...
        <template v-else-if="activeTab === 'more'">
              ...
              <div class="row items-center q-gutter-md">
                <q-avatar
                  ...
                  style="
                    ...
                    font-weight: 700;
                  "
                  >{{ userName.charAt(0).toUpperCase() }}</q-avatar
                >
                <div>
          ...
          <q-card class="q-mb-md" style="border-radius: 16px">
            <q-list style="background: #faf2e4; border-radius: 16px">
              <q-item clickable @click="openAdminFromMore" style="border-radius: 12px 12px 0 0">
                ...
                <q-item-section>
                  ...
                  <q-item-label caption style="color: #9aaa90"
                    >View all users &amp; stats</q-item-label
                  >
                </q-item-section>
                <q-item-section side
...
export default {
  ...
  data() {
    return {
      ...
      // Server-confirmation flags (persisted)
      registeredOnServer: false,
      trackingStartedOnServer: false,
      // Constants
      MOTIVATIONAL_QUOTES,
      HEALTH_MILESTONES,
      EDUCATIONAL_TIPS,
  ...
  methods: {
    ...
    // ─────────────────────────────────────────────────────────
    // APP OPEN — called on every mount (page load / refresh)
    ...
    async _recordAppOpenAndSync() {
      try {
        ...
        console.warn('⚠️ app-open failed:', e.message)
      }
    ...
    // ─────────────────────────────────────────────────────────
    // SYNC — everything except app-open recording
    // Safe to call multiple times (progress updates, daily logs, etc.)
    // ─────────────────────────────────────────────────────────
    async _syncNonOpen() {
      try {
        // Ensure user is registered on server (only once, flag persisted)
        ...
        // Ensure tracking setup is on server (only once, flag persisted)
        ...
        // Progress syncs every open — it changes daily
        if (this.hasStarted) {
        ...
        console.error('_syncNonOpen error:', error)
      }
    },
    ...
    _setupBackgroundSync() {
      ...
      this.backgroundSyncInterval = setInterval(
        async () => {
          if (!this.isOnline || !this.hasStarted || !this.deviceId) return
          try {
            await userAPI.updateProgress(
              this.deviceId,
              this.stats.days,
    ...
    // ─────────────────────────────────────────────────────────
    // Coming back online — sync progress only, NOT app-open
    // ─────────────────────────────────────────────────────────
    async _onOnline() {
      this.isOnline = true
    ...
    async handleRegister(name) {
      ...
      this.registeredOnServer = false // reset so it re-registers
      this._saveToStorage()
      if (this.isOnline) {
        try {
          await userAPI.register(this.deviceId, this.userName)
    ...
    async handleStart({ quitDate, cigarettesPerDay, pricePerPack }) {
      // Normalize date: datetime-local gives "2026-04-27T08:00", server needs "2026-04-27"
      const normalizedDate = quitDate.includes('T') ? quitDate.split('T')[0] : quitDate

      this.quitDate = normalizedDate
      this.cigarettesPerDay = cigarettesPerDay
      this.pricePerPack = pricePerPack
      this.hasStarted = true
      this.trackingStartedOnServer = false // reset so it syncs
      this._quitDateRef.value = normalizedDate
    ...
    onLogDay({ smoked, smokedCount = 0 }) {
      this._log.dailyLogs.value = this.dailyLogs
      this._log.logDay(smoked, smokedCount)
      this._syncLogToData()
      this._recalc()
      const now = new Date()
        ...
        if (val) {
          if (this._isNativePlatform()) {
            ...
            if (display === 'granted') {
              ...
              this.$q.notify({
                color: 'warning',
                message: 'Please allow notifications in your phone settings.',
    ...
    async _loadCommunityMessages() {
      ...
      try {
        ...
        this.encouragementMessages = msgs.map((m) => ({
          id: m.id,
          alias: m.alias,
          text: m.message,
    ...
    async onJoinChallenge() {
      ...
      try {
        ...
        this.$q.notify({
          color: 'positive',
          message: `You joined as "${this.myAlias}"! 🎉`,
          icon: 'groups',
          position: 'center',
          timeout: 2500,
        })
    ...
    async onPostEncouragement() {
      ...
      try {
        if (this.isOnline) {
          ...
          this.$q.notify({
            color: 'warning',
            message: 'You are offline. Connect to post.',
            position: 'center',
            timeout: 2000,
          })
        }
    ...
    _saveToStorage() {
      ...
      LocalStorage.set('quit-smoking-data', {
        ...
        registeredOnServer: this.registeredOnServer,
        trackingStartedOnServer: this.trackingStartedOnServer,
      })
    ...
    _loadFromStorage() {
      ...
      if (d) {
        ...
        this.myAlias = d.myAlias || ''
        // Restore server-confirmation flags so we don't re-send on every open
        this.registeredOnServer = d.registeredOnServer || false
        this.trackingStartedOnServer = d.trackingStartedOnServer || false
        if (this.hasStarted) {
          this._recalc()
          this._startInterval()
        }
...
.admin-toggle-btn {
  background: rgba(168, 210, 168, 0.28) !important;
  color: #5d9460 !important;
...
.reset-btn {
  color: #c97a8a !important;
  border: 1.5px dashed #e8aab8 !important;
...
.reset-btn:hover {
  background: rgba(201, 122, 138, 0.08) !important;
  border-color: #c97a8a !important;
  color: #a85c6e !important;
}
</style>
